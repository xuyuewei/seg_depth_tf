...
class Seg_Depth_Model:
    ...
    def build_model(self,training = False):
        ...
        print('')
        depth_stack = conv_block(tf.layers.conv2d, depth_stack, 32, 3, strides=1,dilation_rate = 1, name='depth_cnn', reg=self.reg)
        print('depth output:')
        depth_stack = conv_block(tf.layers.conv2d, depth_stack, 3, 1, strides=1,dilation_rate = 1, name='depth_stack', reg=self.reg)
    ...
    def CNN_res(self, bottom, filters = [32,64,128,256], reuse = False):
        ...
        with tf.variable_scope(str(dep)+'CNN'):
            with tf.variable_scope(str(dep)+'conv1'):
                bottom = conv_block(tf.layers.conv2d, bottom, filters[0], 3, strides=1,dilation_rate = 1, name=str(dep)+'conv1_0', reuse=reuse, reg=self.reg)
                for i in range(2):
                    bottom = res_block(tf.layers.conv2d, bottom, filters[0], 3, dilation_rate = 1,name=str(dep)+'conv1_%d' % (i+1), reuse=reuse, reg=self.reg)
            if dep < 2:
                return bottom
            with tf.variable_scope(str(dep)+'conv2'):
                bottom = conv_block(tf.layers.conv2d, bottom, filters[1], 3, strides=1,dilation_rate = 1, name=str(dep)+'conv2_0', reuse=reuse, reg=self.reg)
                for i in range(2):
                    bottom = res_block(tf.layers.conv2d, bottom, filters[1], 3, dilation_rate = 1,name=str(dep)+'conv2_%d' % (i+1), reuse=reuse, reg=self.reg)
            if dep < 3:
                return bottom
            with tf.variable_scope(str(dep)+'conv3'):
                bottom = conv_block(tf.layers.conv2d, bottom, filters[2], 3, strides=1,dilation_rate = 1, name=str(dep)+'conv3_0', reuse=reuse, reg=self.reg)
                for i in range(2):
                    bottom = res_block(tf.layers.conv2d, bottom, filters[2], 3, dilation_rate=1, name=str(dep)+'conv3_%d' % (i+1), reuse=reuse,
                                       reg=self.reg)
            if dep < 4:
                return bottom
            with tf.variable_scope(str(dep)+'conv4'):
                bottom = conv_block(tf.layers.conv2d, bottom, filters[3], 3, strides=1,dilation_rate = 1, name=str(dep)+'conv4_0', reuse=reuse, reg=self.reg)
                for i in range(2):
                    bottom = res_block(tf.layers.conv2d, bottom, filters[3], 3, dilation_rate=1, name=str(dep)+'conv4_%d' % (i+1), reuse=reuse,
                                       reg=self.reg)
    ...
    def SPP(self, bottom, reuse=False):
        ...
        with tf.variable_scope('SPP'):
            ...
            for i, p in enumerate([32, 16, 8, 4]):
                branches.append(SPP_branch(tf.layers.conv2d, bottom, p, 128, 3,dilation_rate = 1, name='branch_%d' % (i+1), reuse=reuse,
                                           reg=self.reg))
            concat = tf.concat(branches, axis=-1, name='concat')
            with tf.variable_scope('fusion'):
                bottom = conv_block(tf.layers.conv2d, concat, 64, 3, dilation_rate = 1,name='conv1', reuse=reuse, reg=self.reg)
                fusion = conv_block(tf.layers.conv2d, bottom, 32, 1,dilation_rate = 1,name='conv2', reuse=reuse, reg=self.reg)
        print('Spp_output:')
        print(fusion.shape)
        return fusion
    ...
    def Unet(self,bottom,filters = 32,reuse=False):
        ...
        with tf.variable_scope('Unet'):
            ...
            for i in range(4):
                ...
                bottom = conv_block(tf.layers.conv2d, bottom, filters, kernel_size, strides,dilation_rate, name='conv0'+str(i), reuse=reuse)
                bottom = res_block(tf.layers.conv2d, bottom, filters, kernel_size, strides, dilation_rate,name='rconv0'+str(i), reuse=reuse)
                downconv.append(bottom)
                filters = filters*2
                print(bottom.shape)
            
            bottom = tf.layers.average_pooling2d(bottom, pool_size, pool_size, 'same', name='avg_pool01'+str(i))
            center = conv_block(tf.layers.conv2d, bottom, filters, kernel_size, strides, dilation_rate,name='cconv0'+str(i), reuse=reuse)
            for i in range(4):
                filters = filters//2
                center = conv_block(tf.layers.conv2d_transpose, center, filters, kernel_size,2,name='dconv'+str(i), reuse=reuse)
                print(center.shape)
                center = tf.concat([center,downconv[3-i]], axis=-1, name='uconcat'+str(i))
                center = conv_block(tf.layers.conv2d, center, filters, kernel_size, strides,dilation_rate, name='conv1'+str(i), reuse=reuse)
                center = res_block(tf.layers.conv2d, center, filters, kernel_size, strides,dilation_rate,name='rconv1'+str(i), reuse=reuse)
                
            center = conv_block(tf.layers.conv2d_transpose, center, filters, kernel_size,2, name='oconv1'+str(i), reuse=reuse)
        print('Unet_output:')
        print(center.shape)
        return center
    ...
    def stackedhourglass(self,bottom, filters = [32,64,128], kernel_size = 3, reg=1e-4):
        with tf.variable_scope('Stackedhourglass'):
            ...
            short_cuts = []
            regressions = []
            bottom = conv_block(tf.layers.conv2d, bottom, filters[0], kernel_size, strides=1,dilation_rate = 1,name='stack_0_1', reg=reg)
            bottom = res_block(tf.layers.conv2d, bottom, filters[0], kernel_size, strides=1,dilation_rate = 1,name='stack_0_2', reg=reg)
            ...
            for i in range(3):
                ...
                bottom = conv_block(tf.layers.conv2d, bottom, filters[1], kernel_size, strides=1, dilation_rate = 1,name='stack_%d_1' % (i+1), reg=reg)
                if i == 0:
                    short_cuts.append(bottom)
                    short_cuts.append(bottom)
                else:
                ...
                bottom = conv_block(tf.layers.conv2d_transpose, bottom, filters[1], kernel_size, strides=4, name='stack_%d_3' % (i+1), reg=reg)
                bottom = tf.add(bottom, short_cuts[1], name='rstack_%d' % (i+1))
                short_cuts[2] = bottom
                print(bottom.shape)
                bottom = conv_block(tf.layers.conv2d_transpose, bottom, filters[0], kernel_size, strides=4, name='stack_%d_4' % (i+1), reg=reg)
            ...
            output = conv_block(tf.layers.conv2d, output, 64, kernel_size, strides=1,dilation_rate = 1, name='constack', reg=reg)
        print('Stackedhourglass_output:')
        print(output.shape)
        return output
   
    def train(self, train_data_path,save_path,batch_size = 10,val_ratio = 0.1,learning_rate = 0.005, epochs = 5,retrain = False):
        ...
        leftimg_path_array, rightimg_path_array = img_path_array(train_data_path[0],val_ratio,'0'),img_path_array(train_data_path[1],val_ratio,'1')
        segimg_path_array, depthimg_path_array = img_path_array(train_data_path[2],val_ratio),img_path_array(train_data_path[3],val_ratio)
        num_of_train_samples = len(leftimg_path_array)
        if val_ratio:
            num_of_val_samples = len(leftimg_path_array[1])
            index = np.arange(num_of_val_samples)
            val_left = load_batch_img(leftimg_path_array[1],random_index = index,num_of_val_samples)
            val_right = load_batch_img(rightimg_path_array[1],random_index = index,num_of_val_samples)
            val_seg = load_batch_img(segimg_path_array[1],random_index = index,num_of_val_samples)
            val_depth = load_batch_img(depthimg_path_array[1],random_index = index,num_of_val_samples)
        ...
        for epoch in range(1, epochs+1):  
            for step in range(steps_per_epoch):
                ...
                batch_left = load_batch_img(leftimg_path_array,random_index = random_index[step,batch_size],ran_aug= ran_aug)
                batch_right = load_batch_img(rightimg_path_array,random_index = random_index[step,batch_size],ran_aug = ran_aug)
                batch_seg = load_batch_img(segimg_path_array,random_index = random_index[step,batch_size],ran_aug = ran_aug)
                batch_depth = load_batch_img(depthimg_path_array,random_index = random_index[step,batch_size],ran_aug = ran_aug)
                
                _,loss = self.sess.run([train_optimizer, self.loss],
                                       feed_dict={self.left: batch_left, self.right: batch_right,
                                                  self.seg: batch_seg, self.depth: batch_depth, self.is_training:True})
                ...
                if val_ratio:
                    if step % 1 == 0:
                        metrics = self.sess.run(self.loss,feed_dict={self.left: val_left, self.right: val_right,
                                                                     self.seg: val_seg, self.depth: val_depth,self.is_training:False})
    ...
    def predict(self,img_data,load_path):
        ...
        #next_batch_data = img_data.get_next()
            ...
            seg, depth = self.sess.run([seg_res,depth_stack],feed_dict={self.left: batch_left, self.right: batch_right,self.is_training:False})
    ...
    def finetune(self, train_data_path,save_path,batch_size = 10,val_ratio = 0.1,learning_rate = 0.005, epochs = 5):
        ...
        if val_ratio:
            num_of_val_samples = len(leftimg_path_array[1])
            index = np.arange(num_of_val_samples)
            val_left = load_batch_img(leftimg_path_array[1],random_index = index,num_of_val_samples,ran_aug= False)
            val_right = load_batch_img(rightimg_path_array[1],random_index = index,num_of_val_samples,ran_aug = False)
            val_seg = load_batch_img(segimg_path_array[1],random_index = index,num_of_val_samples,ran_aug = False)
            val_depth = load_batch_img(depthimg_path_array[1],random_index = index,num_of_val_samples,ran_aug = False)
                ...
                if val_ratio:
                    if step % 1 == 0:
                        metrics = self.sess.run(loss,feed_dict={left_img: val_left, right_img: val_right,
                                                                     seg_img: val_seg, depth_img: val_depth,training:False})
                        print('Step %d metrics = %.3f ,training loss = %.3f \n' %(step+1, metrics, loss))
